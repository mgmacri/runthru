...
///
/// - Above [RunThruTiming.a013FallbackWpmThreshold] WPM: falls back to
...
///   [RunThruTiming.a013MaxDisplayFraction] of [displayMs], accounting for
///   per-glyph stagger, and floored at [RunThruTiming.a013MinBaseDuration].
WordTransitionResult selectWordTransition({
  ...
  if (wpm > RunThruTiming.a013FallbackWpmThreshold) {
    return (
      transition: WordTransition.a001Breathe,
      baseDurationMs: RunThruAnimations.wordAdvanceDuration.inMilliseconds,
    );
  }

  // P6 Grade A — at or below threshold, use A-013 with adaptive capping
  final int staggerTotal =
      RunThruAnimations.glyphStaggerMs * math.max<int>(0, charCount - 1);
  final int budget =
  ...
  final int baseDurationMs = math.max<int>(
    RunThruTiming.a013MinBaseDuration,
    budget,
  );
