# Speedy Boy v2.0 — UX/UI Design Specification

**Product**: Speedy Boy v2.0 — Speed Reading with 3D Neumorphic Cube Viewport
**Spec Version**: 1.0.0
**Date**: 2026-04-01
...
Speedy Boy's design exists at the intersection of three evidence-grounded commitments: **cognitive load reduction**, **ambient engagement**, and **executive function externalization**.
...
The existing `SpeedyBoyTokens` in `lib/design/tokens.dart` defines the authoritative color palette. The spec validates this palette against the evidence and adds semantic mappings.
...
| Reading stage (user options) | See `SpeedyBoyTypography.availableFonts` | P10 (Grade B): user sovereignty. 22 options including serif, sans-serif, and monospace. |
...
```
┌──────────────────────────────┐
│   Speedy Boy                 │ ← Title (display style)
│──────────────────────────────│
│                              │
│   ┌──────────┐ ┌──────────┐ │
...
- Cards use `SpeedyBoyDecorations.raisedDecoration(SpeedyBoySurface.shell)`
...
int computeDisplayMs(List<String> words, int index, int wpm, {bool isRareWord = false}) {
  ...
  return SpeedyBoyTiming.wordDisplayMs(
    word,
    wpm: wpm,
...
**Spec change required**: The current `intervalMs` getter uses a flat `60000 / wpm`. This **should** be replaced with the per-word formula from `SpeedyBoyTiming.wordDisplayMs()`, so each word gets its own computed duration. Drift correction must work against the per-word interval, not a fixed interval.
...
2. Compute per-word duration using `SpeedyBoyTiming.wordDisplayMs()`
...
| **Font Family** | Dropdown/picker | `SpeedyBoyTypography.availableFonts` (22 options) | Bricolage Grotesque | P10 (Grade B) |
...
Shell surface tokens exclusively. **Must** use `SpeedyBoySurface.shell` decorations.
...
Motion in Speedy Boy serves three functions, in priority order:
...
**Required change**: `WordTimerNotifier` must integrate per-word display duration from `SpeedyBoyTiming.wordDisplayMs()` instead of using flat `intervalMs`.
...
2. No hardcoded `TextStyle` in widgets — use `SpeedyBoyTypography`
3. No hardcoded `BoxDecoration` shadows — use `SpeedyBoyDecorations`
4. No hardcoded 3D material constants — use `SpeedyBoyMaterials`
...
1. Create `lib/design/timing_tokens.dart` with `SpeedyBoyTiming` class
2. Export through `lib/design/design.dart`
3. Update `AppConfig.defaultWpm` from `300` to `250`
4. Update `WordTimerNotifier` to use `SpeedyBoyTiming.wordDisplayMs()` per word
...
| Neumorphic raised/inset shadows | `SpeedyBoyDecorations` | N/A | Aesthetic | Warm, approachable, non-clinical aesthetic |
...
| **Riverpod** | State management library for Flutter. All global/shared state in Speedy Boy uses Riverpod providers. |
| **go_router** | Declarative routing library for Flutter. All navigation in Speedy Boy uses go_router. |
